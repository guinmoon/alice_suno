...
class SongsGen:
    ...
    def _download_suno_song(self, link: str, song_id: str, output_dir: str) -> str:
        out_mp3 = None
        ...
        out_mp3 = os.path.join(output_dir,f"suno_{song_id}.mp3" )
        with open(out_mp3, "wb") as output_file:
    ...
    def save_songs(
        ...
        try:
            # DEBUG!!!
            # link_list = ['https://audiopipe.suno.ai/?item_id=2e6a028b-3f72-4fc5-a5a8-bccfd88fceaa',
            #          'https://audiopipe.suno.ai/?item_id=a933e770-2da3-4d52-9722-40b2affbc598']
            self.get_songs(
                prompt,
                tags=tags,
                title=title,
        ...
        for _link in link_list:
            if (cur_dwld_ind>=max_dwld_count):
                break
            # self.song_info_dict = _link
            # link = _link['audio_url']
            link = _link
            if link.endswith(".mp3"):            
                mp3_id = link.split("/")[-1][:-4]
            else:
                mp3_id = link.split("=")[-1]
            res = self._download_suno_song(link, mp3_id, output_dir)            
            # !DEBUG!
            # res = "./output/suno_2e6a028b-3f72-4fc5-a5a8-bccfd88fceaa.mp3"
            songs.append(res)
            cur_dwld_ind+=1
        return songs
