...
from aioalice.types import UploadedImage, Quota, UploadedSound
...
class Dispatcher:
    ...
    async def upload_sound(self, image_url_or_bytes, skill_id=None, oauth_token=None):
        """
        ...
        result = await api.request(
            self.session, oauth_token, skill_id,
            api.Methods.SOUNDS, json, file
        )
        if 'sound' not in result:
            raise exceptions.ApiChanged(f'Expected "sounds" in result, got {result}')
        return UploadedSound(**result['sound'])
